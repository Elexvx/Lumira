...
| verify-release-env | release-infra | BLOCKED | `DDD_RELEASE_ENV_FILE=<release-env-file> node scripts/ddd-release-env-file-lint.mjs` | none | `release-env-plan.json` |
| verify-docker-images | release-infra | BLOCKED | `node scripts/ddd-docker-build-evidence.mjs --check` | none | `docker-image-plan.json` |
| verify-runtime | release-infra, frontend, ai | BLOCKED | `node scripts/ddd-staging-runtime-check.mjs` | none | `runtime-smoke-plan.json` |
| verify-data-safety | bounded-context owners, database | BLOCKED | `node scripts/ddd-staging-data-safety-check.mjs` | none | `data-safety-owner-plan.json` |
| verify-final-acceptance | release-infra | BLOCKED | `node scripts/ddd-staging-execution-checklist.mjs --final-review-enforce` | none | `final-review.json` |
