...
- Post-env verification route: blockedPhases=6/6; command=`node scripts/ddd-staging-execution-checklist.mjs --next-action-verification-plan-markdown`
- Release-owner final review: accepted=0/6; command=`node scripts/ddd-staging-execution-checklist.mjs --final-review-enforce`
...
- Deployable Docker images built or inspected: owner=release-infra; blocker=docker CLI is not available: spawnSync docker ENOENT; first=`node scripts/ddd-docker-build-evidence.mjs --check`
...
- Release-owner final review and strict gate: owner=release-owner; blocker=accepted=0/6; first=`node scripts/ddd-staging-execution-checklist.mjs --evidence-acceptance`
