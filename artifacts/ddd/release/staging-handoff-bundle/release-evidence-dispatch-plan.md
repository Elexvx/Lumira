...
| `backend_base_url` | yes | READY | `__REQUIRED_HTTPS__` | `LUMIRA_BASE_URL` |
| `frontend_base_url` | yes | READY | `__REQUIRED_HTTPS__` | `PLAYWRIGHT_BASE_URL` |
| `ai_base_url` | no | READY | `` | `LUMIRA_AI_BASE_URL` |
| `max_artifact_age_hours` | yes | READY | `24` | `operator` |
| `expect_ai_remote` | yes | READY | `true` | `DDD_AI_EXPECT_PROVIDER_REMOTE, DDD_AI_EXPECT_OWNER_GATEWAY_REMOTE` |
| `expect_frontend_deployed` | yes | READY | `true` | `DDD_FRONTEND_EXPECT_DEPLOYED` |
...
| `lane_completion_receipt_base64` | no | BLOCKED | `__REQUIRED_AFTER_COVERAGE_5_OF_5__` | `node scripts/ddd-staging-execution-checklist.mjs --lane-completion-receipt-base64 --lane-completion-receipt-file=<receipt-file>` |
...
Next: `node scripts/ddd-staging-execution-checklist.mjs --owner-evidence-intake-markdown`
