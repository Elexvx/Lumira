...
if [[ "${DDD_RELEASE_NEXT_ACTION_LIST}" == "1" || "${DDD_RELEASE_NEXT_ACTION_LIST}" == "true" ]]; then
  ...
  if [[ ( -z "${DDD_RELEASE_NEXT_ACTION_ORDER}" || "${DDD_RELEASE_NEXT_ACTION_ORDER}" == '1' ) && ( -z "${DDD_RELEASE_NEXT_ACTION_OWNER}" || "${DDD_RELEASE_NEXT_ACTION_OWNER}" == 'release-infra' ) ]]; then
    echo '1 owner=release-infra receiptStatus=CONTENT_BLOCKED next=Replace every placeholder-like value (`<placeholder>`, `replace-with-*`, TODO/TBD, example domains) in `DDD_RELEASE_ENV_FILE` before running release evidence.'
    DDD_RELEASE_NEXT_ACTION_LIST_MATCHED=1
  fi
  if [[ ( -z "${DDD_RELEASE_NEXT_ACTION_ORDER}" || "${DDD_RELEASE_NEXT_ACTION_ORDER}" == '2' ) && ( -z "${DDD_RELEASE_NEXT_ACTION_OWNER}" || "${DDD_RELEASE_NEXT_ACTION_OWNER}" == 'platform-owners' ) ]]; then
    echo '2 owner=platform-owners receiptStatus=CONTENT_BLOCKED next=Set AI_SERVICE_BASE_URL or LUMIRA_AI_SERVICE_BASE_URL or LUMIRA_AI_BASE_URL for ai service in DDD_RELEASE_ENV_FILE or the production-equivalent runtime environment, then rerun `node scripts/ddd-release-config-evidence.mjs`.'
    DDD_RELEASE_NEXT_ACTION_LIST_MATCHED=1
  fi
  if [[ ( -z "${DDD_RELEASE_NEXT_ACTION_ORDER}" || "${DDD_RELEASE_NEXT_ACTION_ORDER}" == '3' ) && ( -z "${DDD_RELEASE_NEXT_ACTION_OWNER}" || "${DDD_RELEASE_NEXT_ACTION_OWNER}" == 'platform-events' ) ]]; then
    echo '3 owner=platform-events receiptStatus=CONTENT_BLOCKED next=Set SAAS_EVENT_REDIS_STREAM_KEY or LUMIRA_EVENT_REDIS_STREAM_KEY for event stream key in DDD_RELEASE_ENV_FILE or the production-equivalent runtime environment, then rerun `node scripts/ddd-release-config-evidence.mjs`.'
    DDD_RELEASE_NEXT_ACTION_LIST_MATCHED=1
  fi
  if [[ ( -z "${DDD_RELEASE_NEXT_ACTION_ORDER}" || "${DDD_RELEASE_NEXT_ACTION_ORDER}" == '4' ) && ( -z "${DDD_RELEASE_NEXT_ACTION_OWNER}" || "${DDD_RELEASE_NEXT_ACTION_OWNER}" == 'ai-owner' ) ]]; then
    echo '4 owner=ai-owner receiptStatus=CONTENT_BLOCKED next=Set LUMIRA_AI_OWNER_INTEGRATIONS_FILE_BASE_URL or LUMIRA_AI_OWNER_FILE_BASE_URL for file owner url in DDD_RELEASE_ENV_FILE or the production-equivalent runtime environment, then rerun `node scripts/ddd-release-config-evidence.mjs`.'
    DDD_RELEASE_NEXT_ACTION_LIST_MATCHED=1
  fi
  if [[ ( -z "${DDD_RELEASE_NEXT_ACTION_ORDER}" || "${DDD_RELEASE_NEXT_ACTION_ORDER}" == '5' ) && ( -z "${DDD_RELEASE_NEXT_ACTION_OWNER}" || "${DDD_RELEASE_NEXT_ACTION_OWNER}" == 'payment-owner' ) ]]; then
    echo '5 owner=payment-owner receiptStatus=CONTENT_BLOCKED next=Set PAYMENT_PUBLIC_BASE_URL for payment public url in DDD_RELEASE_ENV_FILE or the production-equivalent runtime environment, then rerun `node scripts/ddd-release-config-evidence.mjs`.'
    DDD_RELEASE_NEXT_ACTION_LIST_MATCHED=1
  fi
...
if [[ "${DDD_RELEASE_NEXT_ACTION_DETAIL}" == "1" || "${DDD_RELEASE_NEXT_ACTION_DETAIL}" == "true" ]]; then
  ...
  if [[ ( -z "${DDD_RELEASE_NEXT_ACTION_ORDER}" || "${DDD_RELEASE_NEXT_ACTION_ORDER}" == '1' ) && ( -z "${DDD_RELEASE_NEXT_ACTION_OWNER}" || "${DDD_RELEASE_NEXT_ACTION_OWNER}" == 'release-infra' ) ]]; then
    ...
    echo 'next=Replace every placeholder-like value (`<placeholder>`, `replace-with-*`, TODO/TBD, example domains) in `DDD_RELEASE_ENV_FILE` before running release evidence.'
    echo 'reason=release-env-lint:release-env-lint-placeholders unresolvedTemplateKeys=93'
    echo 'readyBatches=p0-docker-release-infra;p0-release-config-release-infra;p0-release-env-lint-release-infra'
  ...
  if [[ ( -z "${DDD_RELEASE_NEXT_ACTION_ORDER}" || "${DDD_RELEASE_NEXT_ACTION_ORDER}" == '3' ) && ( -z "${DDD_RELEASE_NEXT_ACTION_OWNER}" || "${DDD_RELEASE_NEXT_ACTION_OWNER}" == 'platform-events' ) ]]; then
    ...
    echo 'receiptStatus=CONTENT_BLOCKED'
    echo 'next=Set SAAS_EVENT_REDIS_STREAM_KEY or LUMIRA_EVENT_REDIS_STREAM_KEY for event stream key in DDD_RELEASE_ENV_FILE or the production-equivalent runtime environment, then rerun `node scripts/ddd-release-config-evidence.mjs`.'
    echo 'reason=release-config:event stream key placeholder value is not allowed'
  ...
  if [[ ( -z "${DDD_RELEASE_NEXT_ACTION_ORDER}" || "${DDD_RELEASE_NEXT_ACTION_ORDER}" == '5' ) && ( -z "${DDD_RELEASE_NEXT_ACTION_OWNER}" || "${DDD_RELEASE_NEXT_ACTION_OWNER}" == 'payment-owner' ) ]]; then
    ...
    echo 'receiptStatus=CONTENT_BLOCKED'
    echo 'next=Set PAYMENT_PUBLIC_BASE_URL for payment public url in DDD_RELEASE_ENV_FILE or the production-equivalent runtime environment, then rerun `node scripts/ddd-release-config-evidence.mjs`.'
    echo 'reason=release-config:payment public url placeholder value is not allowed'
...
if [[ "${DDD_RELEASE_NEXT_ACTION_CHECK_ENV}" == "1" || "${DDD_RELEASE_NEXT_ACTION_CHECK_ENV}" == "true" ]]; then
  ...
  if [[ ( -z "${DDD_RELEASE_NEXT_ACTION_ORDER}" || "${DDD_RELEASE_NEXT_ACTION_ORDER}" == '2' ) && ( -z "${DDD_RELEASE_NEXT_ACTION_OWNER}" || "${DDD_RELEASE_NEXT_ACTION_OWNER}" == 'platform-owners' ) ]]; then
    DDD_RELEASE_NEXT_ACTION_ENV_CHECK_MATCHED=1
    check_next_action_env '2' 'platform-owners' 'AI_SERVICE_BASE_URL' 'LUMIRA_AI_BASE_URL' 'LUMIRA_AI_SERVICE_BASE_URL' || DDD_RELEASE_NEXT_ACTION_ENV_CHECK_FAILED=1
  fi
  if [[ ( -z "${DDD_RELEASE_NEXT_ACTION_ORDER}" || "${DDD_RELEASE_NEXT_ACTION_ORDER}" == '3' ) && ( -z "${DDD_RELEASE_NEXT_ACTION_OWNER}" || "${DDD_RELEASE_NEXT_ACTION_OWNER}" == 'platform-events' ) ]]; then
    DDD_RELEASE_NEXT_ACTION_ENV_CHECK_MATCHED=1
    check_next_action_env '3' 'platform-events' 'LUMIRA_EVENT_REDIS_STREAM_KEY' 'SAAS_EVENT_REDIS_STREAM_KEY' || DDD_RELEASE_NEXT_ACTION_ENV_CHECK_FAILED=1
  fi
  if [[ ( -z "${DDD_RELEASE_NEXT_ACTION_ORDER}" || "${DDD_RELEASE_NEXT_ACTION_ORDER}" == '4' ) && ( -z "${DDD_RELEASE_NEXT_ACTION_OWNER}" || "${DDD_RELEASE_NEXT_ACTION_OWNER}" == 'ai-owner' ) ]]; then
    DDD_RELEASE_NEXT_ACTION_ENV_CHECK_MATCHED=1
    check_next_action_env '4' 'ai-owner' 'LUMIRA_AI_OWNER_FILE_BASE_URL' 'LUMIRA_AI_OWNER_INTEGRATIONS_FILE_BASE_URL' || DDD_RELEASE_NEXT_ACTION_ENV_CHECK_FAILED=1
  fi
  if [[ ( -z "${DDD_RELEASE_NEXT_ACTION_ORDER}" || "${DDD_RELEASE_NEXT_ACTION_ORDER}" == '5' ) && ( -z "${DDD_RELEASE_NEXT_ACTION_OWNER}" || "${DDD_RELEASE_NEXT_ACTION_OWNER}" == 'payment-owner' ) ]]; then
    DDD_RELEASE_NEXT_ACTION_ENV_CHECK_MATCHED=1
    check_next_action_env '5' 'payment-owner' 'PAYMENT_PUBLIC_BASE_URL' || DDD_RELEASE_NEXT_ACTION_ENV_CHECK_FAILED=1
  fi
...
maybe_run_next_action '1' 'release-infra' 'CONTENT_BLOCKED' 'Replace every placeholder-like value (`<placeholder>`, `replace-with-*`, TODO/TBD, example domains) in `DDD_RELEASE_ENV_FILE` before running release evidence.'
...
maybe_run_next_action '2' 'platform-owners' 'CONTENT_BLOCKED' 'Set AI_SERVICE_BASE_URL or LUMIRA_AI_SERVICE_BASE_URL or LUMIRA_AI_BASE_URL for ai service in DDD_RELEASE_ENV_FILE or the production-equivalent runtime environment, then rerun `node scripts/ddd-release-config-evidence.mjs`.'
if [[ "${DDD_RELEASE_NEXT_ACTION_MATCHED}" == "1" && ( -z "${DDD_RELEASE_NEXT_ACTION_ORDER}" || "${DDD_RELEASE_NEXT_ACTION_ORDER}" == '2' ) && ( -z "${DDD_RELEASE_NEXT_ACTION_OWNER}" || "${DDD_RELEASE_NEXT_ACTION_OWNER}" == 'platform-owners' ) ]]; then
# -----
# Reason: release-config:ai service placeholder value is not allowed
  run_next_action_command '2' 'platform-owners' 'CONTENT_BLOCKED' 'node scripts/ddd-release-config-evidence.mjs'
  run_next_action_command '2' 'platform-owners' 'CONTENT_BLOCKED' 'DDD_RELEASE_OWNER=platform-owners DDD_RELEASE_PRIORITY=P0 DDD_RELEASE_LIST_BATCHES=1 bash artifacts/ddd/release/release-execution-commands.sh'
  run_next_action_command '2' 'platform-owners' 'CONTENT_BLOCKED' 'DDD_RELEASE_OWNER=platform-owners DDD_RELEASE_PRIORITY=P0 DDD_RELEASE_CHECK_ENV_ONLY=1 bash artifacts/ddd/release/release-execution-commands.sh'
  run_next_action_command '2' 'platform-owners' 'CONTENT_BLOCKED' 'DDD_RELEASE_OWNER=platform-owners DDD_RELEASE_PRIORITY=P0 DDD_RELEASE_DRY_RUN=1 bash artifacts/ddd/release/release-execution-commands.sh'
  run_next_action_command '2' 'platform-owners' 'CONTENT_BLOCKED' 'DDD_RELEASE_OWNER=platform-owners DDD_RELEASE_PRIORITY=P0 bash artifacts/ddd/release/release-execution-commands.sh'
fi

maybe_run_next_action '3' 'platform-events' 'CONTENT_BLOCKED' 'Set SAAS_EVENT_REDIS_STREAM_KEY or LUMIRA_EVENT_REDIS_STREAM_KEY for event stream key in DDD_RELEASE_ENV_FILE or the production-equivalent runtime environment, then rerun `node scripts/ddd-release-config-evidence.mjs`.'
if [[ "${DDD_RELEASE_NEXT_ACTION_MATCHED}" == "1" && ( -z "${DDD_RELEASE_NEXT_ACTION_ORDER}" || "${DDD_RELEASE_NEXT_ACTION_ORDER}" == '3' ) && ( -z "${DDD_RELEASE_NEXT_ACTION_OWNER}" || "${DDD_RELEASE_NEXT_ACTION_OWNER}" == 'platform-events' ) ]]; then
# -----
# Reason: release-config:event stream key placeholder value is not allowed
  run_next_action_command '3' 'platform-events' 'CONTENT_BLOCKED' 'node scripts/ddd-release-config-evidence.mjs'
  run_next_action_command '3' 'platform-events' 'CONTENT_BLOCKED' 'DDD_RELEASE_OWNER=platform-events DDD_RELEASE_PRIORITY=P0 DDD_RELEASE_LIST_BATCHES=1 bash artifacts/ddd/release/release-execution-commands.sh'
  run_next_action_command '3' 'platform-events' 'CONTENT_BLOCKED' 'DDD_RELEASE_OWNER=platform-events DDD_RELEASE_PRIORITY=P0 DDD_RELEASE_CHECK_ENV_ONLY=1 bash artifacts/ddd/release/release-execution-commands.sh'
  run_next_action_command '3' 'platform-events' 'CONTENT_BLOCKED' 'DDD_RELEASE_OWNER=platform-events DDD_RELEASE_PRIORITY=P0 DDD_RELEASE_DRY_RUN=1 bash artifacts/ddd/release/release-execution-commands.sh'
  run_next_action_command '3' 'platform-events' 'CONTENT_BLOCKED' 'DDD_RELEASE_OWNER=platform-events DDD_RELEASE_PRIORITY=P0 bash artifacts/ddd/release/release-execution-commands.sh'
fi

maybe_run_next_action '4' 'ai-owner' 'CONTENT_BLOCKED' 'Set LUMIRA_AI_OWNER_INTEGRATIONS_FILE_BASE_URL or LUMIRA_AI_OWNER_FILE_BASE_URL for file owner url in DDD_RELEASE_ENV_FILE or the production-equivalent runtime environment, then rerun `node scripts/ddd-release-config-evidence.mjs`.'
if [[ "${DDD_RELEASE_NEXT_ACTION_MATCHED}" == "1" && ( -z "${DDD_RELEASE_NEXT_ACTION_ORDER}" || "${DDD_RELEASE_NEXT_ACTION_ORDER}" == '4' ) && ( -z "${DDD_RELEASE_NEXT_ACTION_OWNER}" || "${DDD_RELEASE_NEXT_ACTION_OWNER}" == 'ai-owner' ) ]]; then
# -----
# Reason: release-config:file owner url placeholder value is not allowed
  run_next_action_command '4' 'ai-owner' 'CONTENT_BLOCKED' 'node scripts/ddd-release-config-evidence.mjs'
  run_next_action_command '4' 'ai-owner' 'CONTENT_BLOCKED' 'DDD_RELEASE_OWNER=ai-owner DDD_RELEASE_PRIORITY=P0 DDD_RELEASE_LIST_BATCHES=1 bash artifacts/ddd/release/release-execution-commands.sh'
  run_next_action_command '4' 'ai-owner' 'CONTENT_BLOCKED' 'DDD_RELEASE_OWNER=ai-owner DDD_RELEASE_PRIORITY=P0 DDD_RELEASE_CHECK_ENV_ONLY=1 bash artifacts/ddd/release/release-execution-commands.sh'
  run_next_action_command '4' 'ai-owner' 'CONTENT_BLOCKED' 'DDD_RELEASE_OWNER=ai-owner DDD_RELEASE_PRIORITY=P0 DDD_RELEASE_DRY_RUN=1 bash artifacts/ddd/release/release-execution-commands.sh'
  run_next_action_command '4' 'ai-owner' 'CONTENT_BLOCKED' 'DDD_RELEASE_OWNER=ai-owner DDD_RELEASE_PRIORITY=P0 bash artifacts/ddd/release/release-execution-commands.sh'
fi

maybe_run_next_action '5' 'payment-owner' 'CONTENT_BLOCKED' 'Set PAYMENT_PUBLIC_BASE_URL for payment public url in DDD_RELEASE_ENV_FILE or the production-equivalent runtime environment, then rerun `node scripts/ddd-release-config-evidence.mjs`.'
if [[ "${DDD_RELEASE_NEXT_ACTION_MATCHED}" == "1" && ( -z "${DDD_RELEASE_NEXT_ACTION_ORDER}" || "${DDD_RELEASE_NEXT_ACTION_ORDER}" == '5' ) && ( -z "${DDD_RELEASE_NEXT_ACTION_OWNER}" || "${DDD_RELEASE_NEXT_ACTION_OWNER}" == 'payment-owner' ) ]]; then
# -----
# Reason: release-config:payment public url placeholder value is not allowed
  run_next_action_command '5' 'payment-owner' 'CONTENT_BLOCKED' 'node scripts/ddd-release-config-evidence.mjs'
  run_next_action_command '5' 'payment-owner' 'CONTENT_BLOCKED' 'DDD_RELEASE_OWNER=payment-owner DDD_RELEASE_PRIORITY=P0 DDD_RELEASE_LIST_BATCHES=1 bash artifacts/ddd/release/release-execution-commands.sh'
  run_next_action_command '5' 'payment-owner' 'CONTENT_BLOCKED' 'DDD_RELEASE_OWNER=payment-owner DDD_RELEASE_PRIORITY=P0 DDD_RELEASE_CHECK_ENV_ONLY=1 bash artifacts/ddd/release/release-execution-commands.sh'
  run_next_action_command '5' 'payment-owner' 'CONTENT_BLOCKED' 'DDD_RELEASE_OWNER=payment-owner DDD_RELEASE_PRIORITY=P0 DDD_RELEASE_DRY_RUN=1 bash artifacts/ddd/release/release-execution-commands.sh'
  run_next_action_command '5' 'payment-owner' 'CONTENT_BLOCKED' 'DDD_RELEASE_OWNER=payment-owner DDD_RELEASE_PRIORITY=P0 bash artifacts/ddd/release/release-execution-commands.sh'
fi
