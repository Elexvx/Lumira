...
import type { CompetitionRecord, CompetitionStageRecord } from '@/services/competition/types';
...
const ReviewAdminWorkbench = () => {
  ...
  const workspaceStatus = workspaceContext?.workspace?.status;
  const workspaceReadOnly = isCompetitionWorkspaceReadOnly(
    workspaceStatus,
    workspaceContext?.workspace?.readOnly,
  );
  ...
  const canAccessPlans = access.hasPermission('review:plan:manage');
  const canAccessBatches = access.hasPermission('review:batch:create');
  const canAccessAssignments = access.hasPermission('review:assignment:manage');
  const canAccessRoster = access.hasPermission('review:roster:manage');
  const canAccessAggregates = access.hasPermission('review:result:aggregate');
  const canAccessAppeals = access.hasPermission('review:appeal:manage');
  const canAccessAwards = Boolean(workspaceUuid) && access.hasPermission('aiadc:certificate-batch:create');
  const canManagePlans = canMutateCompetitionWorkspace(canAccessPlans, workspaceStatus, workspaceReadOnly);
  const canManageBatches = canMutateCompetitionWorkspace(canAccessBatches, workspaceStatus, workspaceReadOnly);
  const canManageAssignments = canMutateCompetitionWorkspace(canAccessAssignments, workspaceStatus, workspaceReadOnly);
  const canManageRoster = canMutateCompetitionWorkspace(canAccessRoster, workspaceStatus, workspaceReadOnly);
  const canSendNotifications = canMutateCompetitionWorkspace(access.hasPermission('review:notification:send'), workspaceStatus, workspaceReadOnly);
  const canScanCheckin = canMutateCompetitionWorkspace(access.hasPermission('review:checkin:scan'), workspaceStatus, workspaceReadOnly);
  const canAggregate = canMutateCompetitionWorkspace(canAccessAggregates, workspaceStatus, workspaceReadOnly);
  const canFinalize = canMutateCompetitionWorkspace(access.hasPermission('review:result:finalize'), workspaceStatus, workspaceReadOnly);
  const canPublish = canMutateCompetitionWorkspace(access.hasPermission('review:result:publish'), workspaceStatus, workspaceReadOnly);
  const canManageAppeals = canMutateCompetitionWorkspace(canAccessAppeals, workspaceStatus, workspaceReadOnly);
  const canGenerateAwards = canMutateCompetitionWorkspace(canAccessAwards, workspaceStatus, workspaceReadOnly);
  ...
  const loadPlans = useCallback(async (nextCompetitionId?: number, nextStageId?: number) => {
    if (!canAccessPlans || (!workspaceUuid && !nextCompetitionId) || !nextStageId) {
      setPlans([]);
      setPlanId(undefined);
      return;
    }
  ...
  }, [canAccessPlans, reviewApi, workspaceUuid]);

  const loadBatches = useCallback(async (nextPlanId?: number) => {
    if (!canAccessBatches || !nextPlanId) {
      setBatches([]);
  ...
  useEffect(() => {
    ...
    if (canAccessAssignments || canAccessRoster) {
    ...
    return () => { active = false; };
  }, [canAccessAssignments, canAccessRoster, workspaceUuid]);
  ...
  useEffect(() => {
    if (!canAccessAwards || !selectedBatch || selectedBatch.status !== 'PUBLISHED') {
      setAwardRules([]);
  ...
  }, [awardRuleForm, canAccessAwards, reviewApi, selectedBatch]);
    ...
    <Space orientation="vertical" size={16} style={{ width: '100%' }}>
      {workspaceReadOnly ? (
        <Alert
          type="info"
          showIcon
          title="赛事已归档，评审数据仅供查看"
          description="评审方案、分配、结果、申诉与获奖名单均已锁定。"
        />
      ) : null}
      <Card>
        <Space wrap>
      ...
      {selectedBatch && workspaceUuid && canAccessAwards && selectedBatch.status === 'PUBLISHED' && (
        ...
        >
          ...
          />
          <Form form={awardRuleForm} layout="vertical" disabled={workspaceReadOnly}>
          ...
          {canGenerateAwards ? <Space style={{ marginBottom: 16 }}>
            <Button
      ...
      {selectedBatch && canAccessAppeals && (
        <Card
          title="6. 结果申诉处理"
          ...
          <DataTable<ReviewAppeal>
            ...
            columns={[
              ...
              },
              {
                ...
                fixed: 'right',
                width: 110,
                render: (_, record) => record.status === 'SUBMITTED' && canManageAppeals ? (
                  <Button
                    type="link"
                    onClick={() => {
                      setAppealResolution('');
                ...
                ) : <Typography.Text type="secondary">{record.resolution || (record.status === 'SUBMITTED' ? '待处理' : '已处理')}</Typography.Text>,
              },
            ]}
          />
        </Card>
