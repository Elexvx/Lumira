...
const PAYMENT_PROVIDER_FIELD_SCHEMAS: Record<PaymentProviderCode, PaymentFieldConfig[]> = {
  ...
  wechat_pay: [
    ...
    { name: 'platformCertSerialNo', label: t('微信支付公钥 ID / 平台证书序列号', 'WeChat Pay public key ID / platform certificate serial number'), required: true, placeholder: t('公钥 ID 或平台证书序列号', 'Public key ID or platform certificate serial number') },
    { name: 'publicKey', label: t('微信支付平台公钥或平台证书', 'WeChat Pay platform public key or certificate'), required: true, placeholder: t('粘贴微信支付平台公钥或平台证书 PEM', 'Paste the WeChat Pay platform public key or certificate PEM'), inputMode: 'textarea' },
