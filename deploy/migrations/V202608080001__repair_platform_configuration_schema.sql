-- Repair existing installations before configuration writes are governed.
-- The maintenance countdown key was added to the runtime request contract
-- without being added to the versioned online migration chain. Keep this
-- migration idempotent so installations that ran the legacy manual scripts
-- remain safe.
...
-- Backfill metadata after repairing current-value definitions so the first
-- governed write observes one complete platform configuration surface.
