...
for (const fileName of requiredExplainFilesWhenPresent) {
  ...
  assert(row, `${hotPathName} must be documented in doc/28-ddd-hot-path-explain-plan.md`);
  ...
  for (const expectedKey of expectedExplainKeys.get(fileName) || []) {
    ...
    assert(
      ...
      `doc/31-ddd-operational-runbook.md must mention expected EXPLAIN index ${expectedKey}`,
    );
    assert(
      releaseChecklist.includes(expectedKey),
      `doc/34-ddd-release-evidence-checklist.md must mention expected EXPLAIN index ${expectedKey}`,
    );
  }
  ...
  assert(
    ...
    `doc/28-ddd-hot-path-explain-plan.md must mention ${fileName} max rows per scan ${maxRowsPerScan}`,
  );
  assert(
    operationalRunbook.includes(fileName),
    `doc/31-ddd-operational-runbook.md must mention required EXPLAIN artifact ${fileName}`,
  );
  assert(
    releaseChecklist.includes(fileName),
    `doc/34-ddd-release-evidence-checklist.md must mention required EXPLAIN artifact ${fileName}`,
  );
  assert(
    releaseChecklist.includes("rows_examined_per_scan"),
    "doc/34-ddd-release-evidence-checklist.md must mention strict rows_examined_per_scan limits",
  );
}
