...

{
  ...
  artifact.suites[0].file = path.join(repoRoot, "services/lumira-system/target/surefire-reports/TEST-example.xml");
...
{
  const artifact = buildArtifact();
  artifact.build.stdoutTail = `Building jar: ${repoRoot}/services/lumira-admin/target/lumira-server.jar`;
...
{
  const artifact = buildArtifact();
  artifact.modules = artifact.modules.filter((item) => item.module !== "services/lumira-ai");
  assert.deepEqual(validateBackendBuildArtifact(artifact), [
    ...
    "missing module report services/lumira-ai",
  ]);
}

{
  ...
  const issues = validateBackendBuildArtifact(artifact);
  assert(issues.includes("duplicate module report services/lumira-admin"));
}
...

{
  ...
  assert.deepEqual(validateBackendBuildArtifact(artifact), [
    ...
    "services/lumira-admin missing Spring Boot entrypoint src/main/java/com/lumira/server/LumiraServerApplication.java",
    "services/lumira-auth missing target/classes",
    "services/lumira-message missing target jar",
  ]);
}

{
  ...
  assert(issues.includes("services/lumira-auth deployable must be true"));
  assert(issues.includes("services/lumira-auth entrypoint must be src/main/java/com/lumira/auth/AuthServiceApplication.java"));
}

{
  ...
  const issues = validateBackendBuildArtifact(artifact);
  assert(issues.includes("services/lumira-admin jar file path is required"));
  assert(issues.includes("services/lumira-admin jar <unknown> bytes must be positive"));
  assert(issues.includes("services/lumira-admin jar <unknown> sha256 must be 64 hex characters"));
}
