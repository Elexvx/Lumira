...
const releaseGateBlockers = [
  ...
  "release-evidence-orchestrator-preflight-lumira-ui-runtime-base-url: missing deployed lumira-ui base URL",
...
writeJson("release/orchestrator-report.json", {
  ...
  preflight: {
    ...
    checks: [
      ...
      {
        id: "lumira-ui-runtime-base-url",
        status: "BLOCKER",
        detail: "missing deployed lumira-ui base URL",
        envKeys: ["PLAYWRIGHT_BASE_URL", "FRONTEND_BASE_URL"],
      },
      {
        id: "lumira-ui-deployed-expectation",
        status: "BLOCKER",
...
writeJson("payment/payment-webhook-e2e.json", {
  ...
  finalState: {
    order: { status: "PAID" },
    webhookEvents: [
      { eventId: "evt-valid", processed: 1, signatureValid: 1, processMessage: "payment webhook processed" },
      { eventId: "evt-replay", processed: 0, signatureValid: 0, processMessage: "request replayed" },
      { eventId: "evt-bad", processed: 0, signatureValid: 0, processMessage: "signature validation failed" },
    ],
  },
  webhooks: {
...
writeJson("lumira-ui/frontend-smoke.json", {
  status: "FAIL",
...
assert(releaseCutoverOwnerMatrix.owners.some((owner) => owner.owner === "lumira-ui"));
...
assert.deepEqual(
  ...
  ["ai", "database", "lumira-ui", "release-infra"].sort(),
);
assert.equal(
...
const frontendOwnerQueueForCounts = releaseFinalOwnerQueue.ownerQueues.find((owner) => owner.owner === "lumira-ui");
...
const releaseFinalOwnerQueueWaitingDetail = spawnSync("bash", [releaseFinalOwnerQueueCommandsPath], {
  ...
  env: {
    ...
    DDD_FINAL_OWNER_QUEUE_OWNER: "lumira-ui",
...
assert.match(releaseFinalOwnerQueueWaitingDetail.stdout, /order=\d+ owner=lumira-ui status=WAITING/);
...
const frontendOwnerQueue = releaseFinalOwnerQueue.ownerQueues.find((owner) => owner.owner === "lumira-ui");
...
if (process.platform === "win32") {
  ...
  assert.match(releaseFinalOwnerQueueCommandsWaitingStaticEnv.stdout, /owner=lumira-ui status=WAITING/);
...
if (process.platform === "win32") {
  ...
  assert.match(releaseFinalOwnerQueueCommandsExecuteWaiting.stderr, /\[ddd-final-owner-queue\]\[blocked\] owner=lumira-ui status=WAITING/);
  ...
  assert.equal(releaseFinalOwnerQueueWaitingRunReport.ownerFilter, "lumira-ui");
...
assert.doesNotMatch(releaseExecutionCommandsListOwner.stdout, /owner=sql/);
...
assert.doesNotMatch(releaseExecutionCommandsOwnerDryRun.stdout, /owner=sql/);
...
assert.equal(summary.diagnostics.frontendSmoke.actionPlan.owner, "lumira-ui");
...
assert.deepEqual(summary.diagnostics.frontendSmoke.actionPlan.items.map((item) => item.id), [
  "lumira-ui-base-url",
  "lumira-ui-deployed-expectation",
  "lumira-ui-playwright-report",
]);
assert.equal(summary.diagnostics.frontendSmoke.actionPlan.items.find((item) => item.id === "lumira-ui-flow-coverage"), undefined);
...
assert.match(markdown, /owner=lumira-ui pendingItems=3 collapsedItems=2 sources=frontend-smoke=3 collapsedSources=orchestrator=2/);
...
assert.match(markdown, /ownerActionCollapsed: source=orchestrator; id=orchestrator-preflight-lumira-ui-runtime-base-url; coveredBy=frontend-smoke:lumira-ui-base-url/);
