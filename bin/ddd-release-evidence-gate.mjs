...
function validateFrontendSmoke() {
  ...
  if (frontendSmoke.invalid) {
    record("frontend-smoke", "blocker", `invalid JSON: ${frontendSmoke.invalid}`, frontendSmoke.file);
    return;
  }
  record("frontend-smoke", "present", "lumira-ui/frontend-smoke.json", frontendSmoke.file);
  validateFreshArtifact("frontend-smoke", frontendSmoke.data);
  validateProductionEquivalence("frontend-smoke", frontendSmoke.data, "frontend smoke");
  if (isLocalBaseUrl(frontendSmoke.data?.baseUrl)) {
    recordLocalOnlyArtifact("frontend-smoke-environment", frontendSmoke.data.baseUrl);
  }
  const frontendHasConcreteBlockers = Array.isArray(frontendSmoke.data?.blockers) && frontendSmoke.data.blockers.length > 0;
  const frontendContractStrict = strict
    || frontendSmoke.data?.expectDeployed === true
    || frontendSmoke.data?.productionEquivalence?.strict === true;
  for (const issue of validateFrontendSmokeArtifact(frontendSmoke.data, { strict: frontendContractStrict, validateBlockers: true })) {
...
function validateFrontendBuildEvidence() {
  ...
  if (frontendBuild.invalid) {
    record("lumira-ui-build-evidence", "blocker", `invalid JSON: ${frontendBuild.invalid}`, frontendBuild.file);
    return;
  }
  record("lumira-ui-build-evidence", "present", "lumira-ui/lumira-ui-build-evidence.json", frontendBuild.file);
  validateFreshArtifact("lumira-ui-build-evidence", frontendBuild.data);
  validateRuntimeProvenance("lumira-ui-build-evidence", frontendBuild.data);
  for (const issue of validateFrontendBuildArtifact(frontendBuild.data)) {
    record("lumira-ui-build-evidence", "blocker", issue);
  }
}
...
function validateFrontendStaticEvidence() {
  ...
  if (frontendStatic.invalid) {
    record("lumira-ui-static-evidence", "blocker", `invalid JSON: ${frontendStatic.invalid}`, frontendStatic.file);
    return;
  }
  record("lumira-ui-static-evidence", "present", "lumira-ui/lumira-ui-static-evidence.json", frontendStatic.file);
  validateFreshArtifact("lumira-ui-static-evidence", frontendStatic.data);
  validateRuntimeProvenance("lumira-ui-static-evidence", frontendStatic.data);
  for (const issue of validateFrontendStaticArtifact(frontendStatic.data)) {
    record("lumira-ui-static-evidence", "blocker", issue);
  }
}
