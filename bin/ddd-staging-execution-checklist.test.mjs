...
try {
  ...
  assert.match(evidenceAcceptanceMarkdownResult.stdout, /\| runtime-business \| release-infra, lumira-ui, ai, file-owner, job-owner, payment-owner \| no \|/);
  ...
  fs.writeFileSync(populatedNextActionEnv, [
    ...
    "DDD_FRONTEND_DEPLOYMENT_EVIDENCE=https://deploy.example/lumira-ui/1",
  ...
  assert(runtimeBusinessSubmissionPlan.ownerSubmissions.some((item) => item.phase === "lumira-ui-runtime-smoke" && item.owner === "lumira-ui"));
  ...
  assert(runtimeBusinessSubmissionPlan.expectedArtifacts.includes("artifacts/ddd/lumira-ui/frontend-smoke.json"));
  ...
  assert(runtimeBusinessSubmissionPlan.laneReceiptFragment.missingArtifacts.includes("artifacts/ddd/lumira-ui/frontend-smoke.json"));
  ...
  assert.match(runtimeBusinessSubmissionPlanMarkdownResult.stdout, /lumira-ui-runtime-smoke/);
  ...
  const ownerPacketCount = Number(handoffBundleResult.stdout.match(/ownerPackets=(\d+)/)?.[1] || 0);
  assert(ownerPacketCount >= 0);
  ...
  assert(bundleRuntimeSmokePlan.phases.some((phase) => phase.id === "lumira-ui-runtime-smoke"));
