...
const frontendDockerfile = fs.readFileSync(path.join(repoRoot, "deploy", "docker", "lumira-ui.Dockerfile"), "utf8");
assert.match(frontendDockerfile, /\bARG\s+NODE_IMAGE=/, "lumira-ui Dockerfile must allow overriding the Node builder image for registry mirrors");
assert.match(frontendDockerfile, /\bARG\s+NGINX_IMAGE=/, "lumira-ui Dockerfile must allow overriding the nginx runtime image for registry mirrors");
assert.match(frontendDockerfile, /FROM\s+\$\{NODE_IMAGE\}\s+AS\s+builder/, "lumira-ui Dockerfile must use NODE_IMAGE for the builder stage");
assert.match(frontendDockerfile, /FROM\s+\$\{NGINX_IMAGE\}/, "lumira-ui Dockerfile must use NGINX_IMAGE for the runtime stage");
assert.match(frontendDockerfile, /pnpm\s+install\s+--frozen-lockfile/, "lumira-ui Dockerfile must use the frozen pnpm lockfile");
assert.match(frontendDockerfile, /pnpm\s+build/, "lumira-ui Dockerfile must run the production build");
assert.match(frontendDockerfile, /COPY\s+deploy\/nginx\/lumira-ui\.conf\s+\/etc\/nginx\/conf\.d\/default\.conf/, "lumira-ui Dockerfile must install the production nginx config");
assert.match(frontendDockerfile, /COPY\s+--from=builder\s+\/workspace\/lumira-ui\/dist\s+\/usr\/share\/nginx\/html/, "lumira-ui Dockerfile must serve the built dist output");
