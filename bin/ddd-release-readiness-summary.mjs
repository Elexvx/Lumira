...
const fastTrackSourcePolicies = {
  ...
  "frontend-smoke": {
    lane: "lumira-ui-acceptance",
    safetyClass: "required-before-cutover",
    acceleration: "Run deployed Playwright smoke and evidence conversion from the deployed lumira-ui URL.",
  },
  "business-e2e": {
...
function classify(blockerEntry) {
  ...
  if (text.includes("backend-build-evidence-provenance")
    ...
    || text.includes("lumira-ui-build-evidence-provenance")
    || text.includes("lumira-ui-static-evidence-provenance")) {
    return {
  ...
  if (text.includes("release-evidence-orchestrator-preflight-lumira-ui-runtime-base-url")
    || text.includes("release-evidence-orchestrator-preflight-lumira-ui-deployed-expectation")) {
    return {
      category: "frontend-smoke",
      owner: "lumira-ui",
...
function ownerActionRollup(summary) {
  ...
  const rootCauseKeyFor = (owner, source, item = {}) => {
    ...
    const byCheck = {
      ...
      "lumira-ui-deployed-expectation": "lumira-ui-deployed-expectation",
      "lumira-ui-runtime-base-url": "lumira-ui-base-url",
    ...
    const byId = {
      ...
      "docker-daemon": "docker-daemon",
      "lumira-ui-base-url": "lumira-ui-base-url",
      "lumira-ui-deployed-expectation": "lumira-ui-deployed-expectation",
...
function releaseFastTrackArtifact(summary) {
  ...
  const cutoverChecklist = [
    ...
    checklistItem("deployable-images", "Deployable backend/lumira-ui images are built and inspected.", ["deployable-image"], true),
    ...
    checklistItem("runtime-business-acceptance", "AI, lumira-ui, file, job, and payment acceptance evidence is complete.", ["runtime-acceptance", "lumira-ui-acceptance", "business-acceptance"], true),
...
function actionCategoryHint(source) {
  ...
  if (source === "frontend-smoke") return "lumira-ui-runtime";
...
const expectedArtifactsBySource = {
  ...
  "frontend-smoke": ["artifacts/ddd/lumira-ui/frontend-smoke.json", "artifacts/ddd/lumira-ui/playwright-smoke-results.json"],
...
const exitCriteriaBySource = {
  ...
  "frontend-smoke": [
    "Frontend smoke runs against a deployed HTTPS lumira-ui with DDD_FRONTEND_EXPECT_DEPLOYED=true.",
...
function frontendSmokeActionPlan(artifact, strictGate) {
  ...
  if (!baseUrl || productionEquivalence?.issues?.length > 0) {
    add(
      "lumira-ui-base-url",
      "lumira-ui",
      "Run frontend smoke against a deployed HTTPS non-local lumira-ui URL.",
      ["PLAYWRIGHT_BASE_URL", "FRONTEND_BASE_URL", "DDD_FRONTEND_DEPLOYMENT_EVIDENCE"],
      productionEquivalence?.issues?.join("; ") || "missing lumira-ui baseUrl",
    );
  }
  ...
  if (staticCoverage?.present === false || (staticCoverage?.missing || 0) > 0) {
    add(
      "lumira-ui-static-spec-coverage",
      "lumira-ui",
      "Update lumira-ui/tests/e2e/app.spec.ts so every required smoke flow has static coverage.",
      [],
...
function markdown(summary) {
  ...
  if (!frontendDiagnostics) {
    ...
    if (frontendDiagnostics.actionPlan) {
      lines.push(`- actionPlan: owner=${frontendDiagnostics.actionPlan.owner || "lumira-ui"} pendingItems=${frontendDiagnostics.actionPlan.pendingItems ?? 0}`);
      for (const item of frontendDiagnostics.actionPlan.items || []) {
        const envKeys = (item.envKeys || []).length > 0 ? `; envKeys=${item.envKeys.join(",")}` : "";
        const flows = (item.flows || []).length > 0 ? `; flows=${item.flows.join("|")}` : "";
        lines.push(`- lumira-uiAction: ${item.id}; owner=${item.owner || "lumira-ui"}; reason=${item.reason || "missing"}${envKeys}${flows}; action=${item.action || "missing"}`);
      }
    }
  }
