...
function validateFrontendSmokeCore(artifact, { strict = false } = {}) {
  ...
  if ((artifact?.summary?.requiredFlows || 0) > 0) {
    for (const [flow, count] of requiredFlowCounts.entries()) {
      if (count > 1) {
        issues.push(`duplicate required lumira-ui flow ${flow}`);
      }
    }
  ...
  if (strict && artifact?.baseUrl && !isHttpsUrl(artifact.baseUrl)) {
    issues.push("strict release requires HTTPS lumira-ui baseURL evidence");
  }
...
function validateStaticSpecCoverage(staticSpecCoverage, requiredFlows) {
  ...
  if (coverage.length !== requiredFlows.length) {
    issues.push(`lumira-ui static smoke coverage entries=${coverage.length}, expected=${requiredFlows.length}`);
  }
  if ((staticSpecCoverage.covered || 0) !== covered) {
    issues.push(`lumira-ui static smoke covered mismatch: declared=${staticSpecCoverage.covered || 0}, actual=${covered}`);
  }
  if ((staticSpecCoverage.missing || 0) !== missing) {
    issues.push(`lumira-ui static smoke missing mismatch: declared=${staticSpecCoverage.missing || 0}, actual=${missing}`);
  }
  for (const flow of requiredFlows) {
    if (!coverageFlowCounts.has(flow)) {
      issues.push(`lumira-ui static smoke coverage missing required flow ${flow}`);
    }
  }
  for (const [flow, count] of coverageFlowCounts.entries()) {
    if (count > 1) {
      issues.push(`duplicate lumira-ui static smoke coverage entry ${flow}`);
    }
    if (!requiredFlowCounts.has(flow)) {
      issues.push(`unknown lumira-ui static smoke coverage entry ${flow}`);
    }
  }
  for (const entry of coverage) {
    if (!entry?.flow) {
      issues.push("lumira-ui static smoke coverage entry is missing flow");
      continue;
    }
    if (!["covered", "missing"].includes(entry.status)) {
      issues.push(`${entry.flow} lumira-ui static smoke coverage status is invalid`);
    }
    if (entry.status === "missing" && (!entry.reason || typeof entry.reason !== "string")) {
      issues.push(`${entry.flow} missing lumira-ui static smoke coverage must include reason`);
    }
  }
  return issues;
}
