...
const contexts = [
  ...
  {
    name: "Message",
    module: "services/lumira-message",
  ...
  {
    name: "Localization",
    module: "services/lumira-localization",
  ...
  {
    name: "Payment",
    module: "services/lumira-payment",
...
function parseOwnerManifest() {
  const manifestPath = "doc/27-ddd-owner-table-manifest.csv";
...
const splitDoc = exists("doc/29-ddd-physical-split-readiness.md")
  ? read("doc/29-ddd-physical-split-readiness.md")
  : "";
const runbook = exists("doc/31-ddd-operational-runbook.md")
  ? read("doc/31-ddd-operational-runbook.md")
  : "";

recordGlobal(
  "split-gate-document",
  splitDoc ? "pass" : "fail",
  splitDoc ? "doc/29-ddd-physical-split-readiness.md present" : "doc/29-ddd-physical-split-readiness.md missing"
);
...
recordGlobal(
  ...
  exists("services/lumira-system/src/test/java/com/lumira/saas/architecture/DddArchitectureBoundaryTest.java") ? "pass" : "fail",
...
for (const context of contexts) {
  ...
  for (const contract of context.internalContracts) {
    if (contractIsPresent(contract, moduleText, runbook, splitDoc, serviceText("services/lumira-quartz"))) {
  ...
  if (crossDeps.length > 0 && context.module !== "services/lumira-admin") {
  ...
  if (context.module === "services/lumira-system" && context.physicalServiceTarget) {
