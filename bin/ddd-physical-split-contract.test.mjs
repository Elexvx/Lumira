  ...
  const artifact = {
    ...
    contexts: requiredPhysicalSplitContexts.map((context) => ({
      ...
      migrationFiles: context.name === "Payment"
        ? ["services/lumira-payment/src/main/resources/db/migration/payment/V1__payment.sql"]
        : [],
...

{
  ...
  artifact.summary = buildPhysicalSplitSummary(artifact);
  assert(validatePhysicalSplitContract(artifact).includes("IAM module must be services/lumira-system, got services/unknown-service"));
}
