...
export function validateFrontendBuildArtifact(artifact) {
  ...
  if (artifact?.status !== expectedStatus) {
    issues.push(`lumira-ui build status must be ${expectedStatus}, got ${artifact?.status ?? "missing"}`);
  }
  if (artifact?.status !== "PASS") {
    issues.push(`status=${artifact?.status ?? "missing"}`);
  }
  if (artifact?.build?.skipped) {
    issues.push("lumira-ui production build was skipped");
  }
  if (artifact?.build && artifact.build.status !== "PASS") {
    issues.push(`lumira-ui production build status=${artifact.build.status}`);
  }
  if (artifact?.build) {
    if (!artifact.build.command) {
      issues.push("lumira-ui production build command is required");
    }
    if (artifact.build.skipped !== true && artifact.build.exitCode !== 0) {
      issues.push(`lumira-ui production build exitCode=${artifact.build.exitCode ?? "missing"}`);
    }
    if (!Number.isFinite(artifact.build.durationMs) || artifact.build.durationMs < 0) {
      issues.push("lumira-ui production build durationMs must be non-negative");
    }
    validateNoLocalPath("lumira-ui production build stdoutTail", artifact.build.stdoutTail, issues);
    validateNoLocalPath("lumira-ui production build stderrTail", artifact.build.stderrTail, issues);
  }
  if (files === 0 || assets === 0) {
  ...
  if (artifact?.summary?.indexHtmlPresent !== true || !artifact?.entrypoint) {
    ...
    if (!String(artifact.entrypoint.file || "").endsWith("lumira-ui/dist/index.html")) {
      issues.push(`entrypoint file must be lumira-ui/dist/index.html, got ${artifact.entrypoint.file ?? "missing"}`);
    }
  }
  if (!Array.isArray(artifact?.largestFiles)) {
    ...
    for (const [index, file] of artifact.largestFiles.entries()) {
      ...
      if (seenFiles.has(file?.file)) {
        issues.push(`duplicate lumira-ui build file metadata ${file.file}`);
      }
...
export function validateFrontendStaticArtifact(artifact) {
  ...
  if (artifact?.status !== expectedStatus) {
    issues.push(`lumira-ui static status must be ${expectedStatus}, got ${artifact?.status ?? "missing"}`);
  }
  if ((artifact?.summary?.commands || 0) !== results.length) {
    issues.push(`lumira-ui static summary commands mismatch: declared=${artifact?.summary?.commands || 0}, actual=${results.length}`);
  }
  if ((artifact?.summary?.passed || 0) !== actualPassed) {
    issues.push(`lumira-ui static summary passed mismatch: declared=${artifact?.summary?.passed || 0}, actual=${actualPassed}`);
  }
  if ((artifact?.summary?.failed || 0) !== actualFailed) {
    issues.push(`lumira-ui static summary failed mismatch: declared=${artifact?.summary?.failed || 0}, actual=${actualFailed}`);
  }
  if ((artifact?.summary?.skipped || 0) !== actualSkipped) {
    issues.push(`lumira-ui static summary skipped mismatch: declared=${artifact?.summary?.skipped || 0}, actual=${actualSkipped}`);
  }
  if (artifact?.summary?.durationMs !== undefined && artifact.summary.durationMs !== actualDurationMs) {
    issues.push(`lumira-ui static summary durationMs mismatch: declared=${artifact.summary.durationMs}, actual=${actualDurationMs}`);
  }
  const resultByName = new Map();
  for (const result of results) {
    if (!requiredFrontendStaticCommands.includes(result.name)) {
      issues.push(`unknown lumira-ui static command ${result.name ?? "missing"}`);
    }
    if (resultByName.has(result.name)) {
      issues.push(`duplicate lumira-ui static command ${result.name}`);
    }
  ...
  for (const commandName of requiredFrontendStaticCommands) {
    ...
    if (!result) {
      issues.push(`missing lumira-ui static command ${commandName}`);
      continue;
    }
    if (result.skipped) {
