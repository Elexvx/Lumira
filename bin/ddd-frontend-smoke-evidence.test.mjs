...
const directory = fs.mkdtempSync(path.join(os.tmpdir(), "frontend-smoke-evidence-"));
...
const repoDirectory = path.join(repoRoot, "tmp", `frontend-smoke-evidence-${process.pid}`);
...

{
  const result = spawnSync("node", ["bin\/ddd-frontend-smoke-evidence.mjs"], {
    ...
    env: {
      ...
      DDD_FRONTEND_DEPLOYMENT_EVIDENCE: "artifact://deployments/lumira-ui/rc-1",
      DDD_EVIDENCE_ENVIRONMENT: "staging",
      DDD_RELEASE_CANDIDATE: "rc-1",
      DDD_EVIDENCE_OPERATOR: "release-owner",
    },
...
try {
  ...
  assert.equal(artifact.diagnostics.staticSpecCoverage.file, "lumira-ui/tests/e2e/app.spec.ts");
  assert(!JSON.stringify(artifact).includes(repoRoot));
} finally {
