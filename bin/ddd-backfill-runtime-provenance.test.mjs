...
{
  const root = makeRoot();
  writeJson(path.join(root, "lumira-ui", "frontend-smoke.json"), {
  ...
  const result = run({
    DDD_RELEASE_EVIDENCE_DIR: root,
    DDD_RUNTIME_PROVENANCE_BACKFILL: "true",
    DDD_RUNTIME_PROVENANCE_BACKFILL_ARTIFACTS: "lumira-ui/frontend-smoke.json",
    DDD_EVIDENCE_ENVIRONMENT: "local-dev",
    DDD_RELEASE_CANDIDATE: "abc123",
    DDD_EVIDENCE_OPERATOR: "codex",
  });
  assert.equal(result.status, 0, result.stderr);
  const artifact = readJson(path.join(root, "lumira-ui", "frontend-smoke.json"));
