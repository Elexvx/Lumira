...
writeJson("build/docker-image-evidence.json", {
  ...
  images: [
    ...
    {
      name: "lumira-ui",
      dockerfile: "deploy/docker/lumira-ui.Dockerfile",
      dockerfileSha256: "b".repeat(64),
      tag: "lumira/lumira-ui:test",
...
const physicalSplitArtifact = {
  ...
  strict: false,
  globalChecks: [
    { name: "split-gate-document", status: "pass", detail: "doc/29-ddd-physical-split-readiness.md present" },
...
const advisoryFrontendFile = path.join(advisoryArtifactRoot, "lumira-ui", "frontend-smoke.json");
