...
if [[ "$1" == "image" && "$2" == "inspect" ]]; then
  ...
  if [[ "$tag" == *"/lumira-server:"* ]]; then
    ...
    printf '%s\\n' '{"Id":"sha256:lumira-ui","RepoTags":["lumira/lumira-ui:test"],"Size":123456,"Created":"2026-01-01T00:00:00Z","Architecture":"arm64","Os":"linux","Config":{"User":"","Entrypoint":[],"Cmd":["nginx"],"WorkingDir":"","ExposedPorts":{"80/tcp":{}}}}'
  fi
  exit 0
fi
...
assert.equal(retryArtifact.images.find((image) => image.name === "lumira-ui").buildArgs.NODE_IMAGE, "registry.local/node:22-bookworm-slim");
assert.equal(retryArtifact.images.find((image) => image.name === "lumira-ui").buildArgs.NGINX_IMAGE, "registry.local/nginx:1.29-alpine");
...
if [[ "$1" == "image" && "$2" == "inspect" ]]; then
  ...
  elif [[ "$tag" == "registry.local/lumira-ui:rc1" ]]; then
    printf '%s\\n' '{"Id":"sha256:lumira-ui-existing","RepoTags":["registry.local/lumira-ui:rc1"],"Size":123456,"Created":"2026-01-01T00:00:00Z","Architecture":"arm64","Os":"linux","Config":{"User":"","Entrypoint":[],"Cmd":["nginx"],"WorkingDir":"","ExposedPorts":{"80/tcp":{}}}}'
  else
...
for (const image of existingImageArtifact.images) {
  ...
  assert.equal(image.tag, image.name === "lumira-server" ? "registry.local/lumira-server:rc1" : "registry.local/lumira-ui:rc1");
