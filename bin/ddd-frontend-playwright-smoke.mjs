...
if (result.error) {
  ...
  console.error(`[ddd-lumira-ui-playwright-smoke] failed to start Playwright smoke: ${result.error.message}`);
  process.exit(1);
}
...
if (!stdout) {
  ...
  console.error("[ddd-lumira-ui-playwright-smoke] Playwright did not emit JSON report on stdout");
  process.exit(1);
}
...
try {
  ...
  console.error(`[ddd-lumira-ui-playwright-smoke] Playwright emitted invalid JSON: ${error.message}`);
  process.exit(1);
}
...
if (result.status !== 0) {
  console.error(`[ddd-lumira-ui-playwright-smoke] Playwright smoke failed; exitCode=${result.status}; report=${outputFile}`);
  process.exit(result.status || 1);
}

console.log(`[ddd-lumira-ui-playwright-smoke] Playwright smoke passed; grep=${grep}; report=${outputFile}`);
