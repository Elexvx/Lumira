...

{
  ...
  assert(issues.includes(`duplicate required lumira-ui flow ${duplicateFlow}`));
...

{
  ...
  assert.deepEqual(artifact.blockers, [
    ...
    "strict release requires HTTPS lumira-ui baseURL evidence",
...

{
  ...
  const issues = validateFrontendSmokeArtifact(artifact);
  assert(issues.includes(`lumira-ui static smoke coverage entries=2, expected=${defaultRequiredFrontendFlows.length}`));
  assert(issues.includes("lumira-ui static smoke covered mismatch: declared=0, actual=1"));
  assert(issues.includes("lumira-ui static smoke missing mismatch: declared=0, actual=1"));
  assert(issues.includes(`lumira-ui static smoke coverage missing required flow ${defaultRequiredFrontendFlows[1]}`));
  assert(issues.includes("unknown lumira-ui static smoke coverage entry unknown flow"));
}
