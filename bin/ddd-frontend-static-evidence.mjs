...
const outputFile = process.env.DDD_FRONTEND_STATIC_REPORT
  ...
  : path.join(outputDir, "lumira-ui-static-evidence.json");
...
const commands = [
  { name: "lint", args: ["pnpm", "--dir", "lumira-ui", "lint"] },
  { name: "typecheck", args: ["pnpm", "--dir", "lumira-ui", "typecheck"] },
  { name: "unit", args: ["pnpm", "--dir", "lumira-ui", "test"] },
];
...
if (strictEvidence) {
  for (const issue of collectProvenanceIssues({ sourceEnvironment, releaseCandidate, evidenceOperator })) {
    blockers.push(`lumira-ui static provenance ${issue}`);
  }
}
...
if (blockers.length > 0) {
  for (const blocker of blockers) {
    console.error(`[ddd-lumira-ui-static-evidence] ${blocker}`);
  }
  console.error(`[ddd-lumira-ui-static-evidence] wrote artifact to ${outputFile}`);
  process.exit(1);
}

console.log(`[ddd-lumira-ui-static-evidence] lumira-ui static evidence passed; commands=${results.length}; artifact=${outputFile}`);
