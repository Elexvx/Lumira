...
if (strictEvidence) {
  for (const issue of collectProvenanceIssues({ sourceEnvironment, releaseCandidate, evidenceOperator })) {
    blockers.push(`lumira-ui build provenance ${issue}`);
  }
}
if (!fs.existsSync(frontendDir)) {
  blockers.push("missing lumira-ui directory");
}

const build = blockers.length === 0 ? runBuild() : null;
if (build && build.status !== "PASS") {
  blockers.push(`lumira-ui build failed: status=${build.status}, exitCode=${build.exitCode}`);
}
if (strictEvidence && build?.skipped) {
  blockers.push("lumira-ui production build was skipped; strict release requires a fresh production build");
}
...
if (!fs.existsSync(distDir)) {
  blockers.push("missing lumira-ui/dist after build");
}
if (!fs.existsSync(indexFile)) {
  blockers.push("missing lumira-ui/dist/index.html after build");
}
if (assets.length === 0) {
  blockers.push("lumira-ui/dist has no static assets");
}
...
if (blockers.length > 0) {
  for (const blocker of blockers) {
    console.error(`[ddd-lumira-ui-build-evidence] ${blocker}`);
  }
  console.error(`[ddd-lumira-ui-build-evidence] wrote artifact to ${outputFile}`);
  process.exit(1);
}

console.log(`[ddd-lumira-ui-build-evidence] lumira-ui build evidence passed; files=${files.length}; artifact=${outputFile}`);
