...
const localEnv = {
  ...
  SAAS_EVENT_OUTBOX_RELAY_ENABLED:
    process.env.SAAS_EVENT_OUTBOX_RELAY_ENABLED
    ?? fileEnv.SAAS_EVENT_OUTBOX_RELAY_ENABLED
    ?? (full ? 'true' : 'false'),
  SAAS_EVENT_OUTBOX_DISPATCHER:
    process.env.SAAS_EVENT_OUTBOX_DISPATCHER
    ?? fileEnv.SAAS_EVENT_OUTBOX_DISPATCHER
    ?? (full ? 'redis-stream' : 'logging'),
