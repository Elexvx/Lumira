...
def main() -> None:
    ...
    shutil.copy2(ROOT / "frontend" / "assets" / "sms-plugin.js", package_dir / "frontend" / "assets" / "sms-plugin.js")
    shutil.copy2(ROOT / "migrations" / "V1__sms_init.sql", package_dir / "migrations" / "V1__sms_init.sql")
    ...
    zip_path = dist_dir / "sms-plugin-1.0.0.zip"
    if zip_path.exists():
        zip_path.unlink()
...
def build_backend_jar(build_dir: Path) -> Path:
    ...
    jar_path = ROOT / "backend" / "target" / "sms-plugin-backend-1.0.0.jar"
...
def create_checksums(package_dir: Path) -> dict[str, str]:
    ...
    for relative_path in [
        ...
        "frontend/assets/sms-plugin.js",
        "migrations/V1__sms_init.sql",
    ]:
