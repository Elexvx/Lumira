...
public class SecurityRateLimitFilter extends OncePerRequestFilter {
    ...
    private boolean isAuthenticationAttemptPath(String path) {
        ...
        return normalized.endsWith("/auth/login")
                ...
                || normalized.endsWith("/auth/registration/contact/availability")
                || normalized.endsWith("/auth/registration/code/challenge")
                || normalized.endsWith("/auth/registration/complete")
