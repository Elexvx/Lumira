...
interface AiIamQueryFacade {
    ...
    UserSearchResult searchUsers(CurrentUser actor, String keyword, String status, int limit);
...
class DefaultAiIamQueryFacade implements AiIamQueryFacade {
    ...
    public UserSearchResult searchUsers(CurrentUser actor, String keyword, String status, int limit) {
        ...
        AiIamUserRepository.UserSearch search = userRepository.search(actor, keyword, status, safeLimit);
        List<Map<String, Object>> users = search.items().stream()
                .map(this::maskedUser)
                .toList();
        return new UserSearchResult(users, search.total());
    }
