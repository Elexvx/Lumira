...
class PluginSecurityAssemblyTest {

    private final ApplicationContextRunner contextRunner = new ApplicationContextRunner()
            .withUserConfiguration(TestConfiguration.class)
            .withPropertyValues(
                    "lumira.monolith=false",
                    "saas.security.jwt-secret=plugin-security-jwt-secret-2026-0123456789abcdef",
                    "saas.internal.auth-token=auth-token-2026"
            );

    @Test
    void splitRuntimeCreatesPluginJwtSecurityBeansWithSharedAuthClient() {
        contextRunner.run(context -> {
            assertThat(context).hasNotFailed();
            assertThat(context.getBeansOfType(AuthInternalApi.class)).hasSize(1);
            assertThat(context.getBeansOfType(PluginJwtAuthFilter.class)).hasSize(1);
        });
    }
