...
public class JdbcReviewRepository implements ReviewRepository {
    ...
    public JdbcReviewRepository(MyBatisQueryOperations database) {
    ...
    public List<ExpertWorkload> listApprovedExpertWorkloads() {
        return database.query(
                """
                        select expert.id as expertId, count(assignment.id) as activeAssignmentCount
                          from aiadc_expert expert
                          left join competition_review_assignment assignment
                            on assignment.expert_id = expert.id
                         ...
                         where expert.status = 'active'
                           ...
                           and expert.user_id is not null
                           and expert.user_uuid is not null
                           and expert.deleted = 0
                         group by expert.id, expert.sort
                         order by activeAssignmentCount asc, expert.sort asc, expert.id asc
                        """,
        ...
        return database.query(
                """
                        select candidate.id as candidateId,
                               ...
                               expert.user_id as expertUserId,
                               expert.user_uuid as expertUserUuid,
                               expert.status as expertStatus,
                               expert.approval_status as approvalStatus,
                               expert.account_status as accountStatus,
                               case
                                   when expert.user_uuid is not null
                                    and json_search(candidate.snapshot_json, 'one', expert.user_uuid) is not null
                                   then 1 else 0
                               end as identityConflict
                          from competition_review_candidate candidate
                          join aiadc_expert expert
                            on expert.id = ? and expert.deleted = 0
    ...
    @Override
    public List<ReviewVO.PublishedResult> listOwnedPublishedResults(Long userId, String userUuid) {
        return database.query(
                """
