...
/**
 * Application implementation for the internal System ports.
 *
 * <p>The HTTP controller is a thin transport adapter that subclasses this
 * service so the existing endpoint annotations remain compatible while local
 * callers resolve the port without depending on a controller.</p>
 */
@Service("systemInternalApplicationPort")
public class SystemInternalApplicationService implements SystemInternalApplicationPort {

    private static final Logger log = LoggerFactory.getLogger(SystemInternalApplicationService.class);
    ...
    @ModelAttribute
    public void requireInternalServicePrincipal() {
    ...
    @GetMapping("/users/enabled-directory")
    @Override
    public List<SystemUserDirectoryEntryDTO> enabledUserDirectory() {
        requireInternalServicePrincipal();
        return internalSystemApplicationService.findEnabledUserDirectory();
    }
