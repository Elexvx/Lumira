...
class JobReadinessV2ControllerTest {
    ...
    void readiness_shouldExposeTablelessAdapterContract() {
        ...
        assertThat(readiness.metrics()).contains("job.lumira-backend_target.configured_count", "job.owner_handler.declared_count");
        ...
        assertThat(metrics.metrics()).anySatisfy(metric -> {
            assertThat(metric.name()).isEqualTo("job.lumira-backend_target.configured_count");
