...
/** Competition-local XLSX renderer; generic task ownership remains in System. */
@Service
public class CompetitionExcelExportService {
    ...
    public <T> byte[] export(String sheetName, List<ExportColumn<T>> columns, List<T> rows) {
        ...
        try (XSSFWorkbook workbook = new XSSFWorkbook(); ByteArrayOutputStream output = new ByteArrayOutputStream()) {
            ...
            for (int index = 0; index < columns.size(); index += 1) {
                ExportColumn<T> column = columns.get(index);
                Cell cell = header.createCell(index);
                cell.setCellValue(column.label());
                cell.setCellStyle(headerStyle);
                sheet.setColumnWidth(index, Math.min(40, Math.max(12, column.label().length() + 8)) * 256);
            }
            ...
            throw new BizException(ErrorCode.BIZ_ERROR, "Failed to generate export file");
        }
    }
    ...
    private String formatValue(Object value) {
        ...
        if (value instanceof Collection<?> collection) {
            ...
            for (Object item : collection) if (item != null) joiner.add(String.valueOf(item));
    ...
    private String safeSheetName(String sheetName) {
        if (sheetName == null || sheetName.isBlank()) return "Export";
        String sanitized = sheetName.replaceAll("[\\\\/?*\\[\\]:]", "");
        if (sanitized.isBlank()) return "Export";
