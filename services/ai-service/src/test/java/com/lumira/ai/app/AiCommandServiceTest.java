...
class AiCommandServiceTest {
    ...
    void searchKnowledgeBoundsLimitAndReturnsRows() {
        ...
        AiCommandService service = new AiCommandService(jdbcTemplate, readQueryService, noOpGateway(), noOpProvider(), new ObjectMapper());
    ...
    void executeLocalPermissionSnapshotTool() {
        JdbcTemplate jdbcTemplate = mock(JdbcTemplate.class);
        AiCommandService service = new AiCommandService(jdbcTemplate, new AiReadQueryService(jdbcTemplate), noOpGateway(), noOpProvider(), new ObjectMapper());
