...
public class JobReadinessV2Controller {
    ...
    public ApiResponse<OwnerObservabilityDTO> health() {
        return ApiResponse.success(new OwnerObservabilityDTO(
                ...
                List.of(
                        ...
                        healthCheck("job.backend-targets.config", configuredTargetCount() > 0 ? "CONFIGURED" : "MISSING", "BackendJobClient has at least one owner target URL configured."),
    ...
    private List<OwnerObservabilityDTO.MetricDTO> jobMetrics() {
        return List.of(
                ...
                metric("job.backend_target.configured_count", "gauge", "targets", "Configured BackendJobClient target URLs.", configuredTargetCount()),
