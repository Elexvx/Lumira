...
- `scripts/ddd-performance-smoke.mjs` 负责 HTTP 热路径冒烟。
- `scripts/ddd-performance-smoke.mjs` 支持 `DDD_PERF_BASELINE_FILE` 与 `DDD_PERF_ACTUAL_FILE`。提供 baseline 后，整体 p95 和逐端点 p95 超过基线 10%（可用 `DDD_PERF_MAX_P95_REGRESSION_RATIO` 调整）会直接失败。基线格式参考 `docs/30-ddd-performance-baseline.example.json`。
- `scripts/ddd-performance-smoke.mjs` 支持 `DDD_SMOKE_SCENARIOS_FILE`，可用 JSON 场景声明 `method`、`path`、`headers`、`body`、`multipart` 和 `expectedStatuses`，用于登录、文件上传、支付 webhook、AI chat 等非 GET 热路径。场景格式参考 `docs/32-ddd-performance-scenarios.example.json`。
- 无运行服务的 CI 先执行 `node --check scripts/ddd-performance-smoke.mjs`，有可访问环境时执行真实 smoke，并保存输出作为 `DDD_PERF_ACTUAL_FILE` 参与回归比较。
- `scripts/ddd-collect-explain.mjs` 负责在可连接 MySQL 的环境执行 `EXPLAIN FORMAT=JSON`，默认写入 `tmp/ddd-explain`，可通过 `DDD_EXPLAIN_DIR` 指定产物目录。数据库名可用 `DDD_EXPLAIN_DATABASE` 或 `MYSQL_DATABASE` 指定，CLI 可用 `MYSQL_CLI` 覆盖，连接参数可用 `MYSQL_HOST`、`MYSQL_PORT`、`MYSQL_USER`、`MYSQL_PASSWORD` 指定。
- `scripts/ddd-explain-gate.mjs` 已接入 CI，默认校验本清单的热点覆盖和字段完整性；当 `DDD_EXPLAIN_DIR` 指向 `EXPLAIN FORMAT=JSON` 产物目录时，会阻断 `access_type=ALL` 或未命中索引的执行计划，并要求 Message/File owner relay 产物命中 `idx_platform_event_outbox_owner_queue`。
- 数据库可用的 CI 环境增加 `node scripts/ddd-collect-explain.mjs && node scripts/ddd-explain-gate.mjs`，由 gate 统一判断失败条件。
- 每次新增 owner 表或 read model，先更新 `docs/27-ddd-owner-table-manifest.csv`，再补本文件的 explain 清单。
