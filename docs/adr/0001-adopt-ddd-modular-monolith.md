...
Lumira currently runs as a modular monolith through `services/lumira-server`, while preserving Maven service modules such as `system-service`, `auth-service`, `message-service`, `file-service`, `plugin-service`, `localization-service`, and `payment-service`.
...
We will keep `services/lumira-server` as the primary runtime entrypoint, and evolve each business module toward explicit bounded contexts. Physical microservice extraction remains a later operational decision, not the first step.
...
- `docs/13-service-data-ownership.md`
- `docs/14-system-service-module-boundaries.md`
- `docs/16-event-outbox-architecture.md`
