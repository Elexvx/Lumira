...
当前后端正式运行形态是单体微服务：只保留 `services/lumira-server` 作为 Spring Boot 启动入口，`services/*-service` 目录作为业务模块 jar 被聚合进同一个进程。

## 当前约束

- 只有 `services/lumira-server` 可以包含 `*Application.java` 启动类。
- 只有 `services/lumira-server/src/main/resources/application.yml` 可以承载应用级配置、端口、Nacos、Sentinel、数据库、Redis、Flyway、Actuator、SpringDoc 等运行配置。
...
这些模块仍应保持清晰的包边界、数据 owner 边界和 API DTO 边界。新增跨模块调用优先通过 `libs/lumira-api` 或明确的应用服务接口表达，避免直接穿透其他模块的内部实体和 mapper。
...
2. 在目标模块新增 `src/main/resources/application.yml`，从 `services/lumira-server/src/main/resources/application.yml` 中复制该模块需要的配置前缀。
